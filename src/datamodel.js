...
class Datamodel {
    ...
    /**
     * The JSON {@link Dataset} object represents the datamodel information of the given datatype `typeName`
     * 
    ...
    /**
     * @returns {string} preset name of the headerfile (this file is not automatically generated by the {@link Datamodel}, but the contents can be accessed via the {@link headerFileCode} property)
     */
    get headerFileName() {
        return `exos_${this.typeName.toLowerCase()}.h`;
    }
    ...
    /**
     * @returns {string} preset name of the source code file (this file is not automatically generated by the {@link Datamodel}, but the contents can be accessed via the {@link sourceFileCode} property)
     */
    get sourceFileName() {
        return `exos_${this.typeName.toLowerCase()}.c`;
    }

    /**
     * @returns {string} an ASCII string with C-datatype definitions of the datatype `typeName` - this is part of the {@link headerFileCode}
     * 
    ...
    */
    _makeSource() {
        ...
        function _infoChildren(children, parent, parentArray) {
            let out = "";
                    ...
                    if (child.name == "variable" || child.name == "enum") {
                        ...
                        if (parent == "") {
                            if (child.attributes.arraySize > 0) {
                                ...
                                child.attributes.info2 = "<infoId" + infoId + ">";
                                out += checkExosInfoCallParam(`        {EXOS_DATASET_BROWSE_NAME(${child.attributes.name}[0]),{${arrayStr}}},\n`);
                                out += _infoChildren(child.children, `${child.attributes.name}[0]`, arrayStr);
                            }
                            else {
                                out += checkExosInfoCallParam(`        {EXOS_DATASET_BROWSE_NAME(${child.attributes.name}),{${arrayStr}}},\n`);
                                out += _infoChildren(child.children, child.attributes.name, arrayStr);
                            }
                        }
                        else {
                            if (child.attributes.arraySize > 0) {
                                ...
                                child.attributes.info2 = "<infoId" + infoId + ">";
                                out += checkExosInfoCallParam(`        {EXOS_DATASET_BROWSE_NAME(${parent}.${child.attributes.name}[0]),{${arrayStr}}},\n`);
                                out += _infoChildren(child.children, `${parent}.${child.attributes.name}[0]`, arrayStr);
        
                            }
                            else {
                                out += checkExosInfoCallParam(`        {EXOS_DATASET_BROWSE_NAME(${parent}.${child.attributes.name}),{${arrayStr}}},\n`);
                                out += _infoChildren(child.children, `${parent}.${child.attributes.name}`, arrayStr);
                            }
                        }
                    }
                }
        ...
        this.#_types.attributes.info = "<infoId" + infoId + ">"; // top level
        let info = _infoChildren(this.#_types.children, "", ""); // needs to be called before JSON.stringify to generate infoId
        let out = "";
    ...
    _makeJsonTypes() {
        ...
        */
        ...
        return _parseTyp(this.#_fileLines, "<NAME>", this.typeName, "", 0);
    }
