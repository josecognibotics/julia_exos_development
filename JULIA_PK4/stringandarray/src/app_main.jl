...
function julia_datamodel_connect_stringarray(datamodel::Ref{julia_exos_datamodel_handle}, datamodel_event_cb::Ptr{Cvoid})
	...
	@ccall libexos_api.exos_datamodel_calc_dataset_info(datasets::Ref{NTuple{4, julia_exos_dataset_info}}, datasets_size::Csize_t)::Cvoid
    
    println("Datasets array after calc")
    print_datasetinfo(datasets)

    return @ccall libexos_api.exos_datamodel_connect(datamodel::Ref{julia_exos_datamodel_handle}, config_stringandarray::Cstring, datasets::Ref{NTuple{4, julia_exos_dataset_info}}, datasets_size::Csize_t, datamodel_event_cb::Ptr{Cvoid})::Cint
end
...
function main()                                                                                                                       
	...
	println("LOG HANDLE before INIT")
    print_log(log_handle[])

    log_init = @ccall libexos_api.exos_log_init(ptr_log_handle::Ref{julia_exos_log_handle}, "gStringAndArray_0"::Cstring)::Cint
    ...
    * This function initializes the `exos_dataset_handle_t` structure, meaning it zeroes all members and sets artefact, data and size members.
    ...
    dataset_init1 = @ccall libexos_api.exos_dataset_init(myint1::Ref{julia_exos_dataset_handle}, stringandarray_handle::Ref{julia_exos_datamodel_handle}, "MyInt1"::Cstring, ptr_MyInt1::Ptr{Cvoid}, sizeof(data[].MyInt1)::Csize_t)::Cint
    ...
    * If the dataset is connected, it will be disconnected before being deleted.
    ...
    dataset_init2 = @ccall libexos_api.exos_dataset_init(myint2::Ref{julia_exos_dataset_handle}, stringandarray_handle::Ref{julia_exos_datamodel_handle}, "MyInt2"::Cstring, ptr_MyInt2::Ptr{Cvoid}, sizeof(data[].MyInt2)::Csize_t)::Cint
    ...
    datamodel_connect_string = unsafe_string(@ccall libexos_api.exos_get_error_string(EXOS_ERROR_CODE(datamodel_connect)::Ref{EXOS_ERROR_CODE})::Cstring)
	...
	* Connect a dataset to the Dataset Message Router and register an event callback
	=#
	#dataset_connect_MyInt1 = @ccall libexos_api.exos_dataset_connect(myint1::Ref{julia_exos_dataset_handle}, EXOS_DATASET_SUBSCRIBE::Cint, dataset_event_cb::Ptr{Cvoid})::Cint
	#dataset_connect_string1 = unsafe_string(@ccall libexos_api.exos_get_error_string(EXOS_ERROR_CODE(dataset_connect_MyInt1)::EXOS_ERROR_CODE)::Ptr{Cchar})
	#println("dataset_connect_MyInt1\t\t\t-> ERROR_CODE: $dataset_connect: $dataset_connect_string1")

	# dataset_connect_MyInt2 = @ccall libexos_api.exos_dataset_connect(myint2::Ref{julia_exos_dataset_handle}, EXOS_DATASET_PUBLISH::EXOS_DATASET_TYPE, datamodel_event_cb::Ptr{Cvoid})::Cint
	# dataset_connect_string2 = unsafe_string(@ccall libexos_api.exos_get_error_string(EXOS_ERROR_CODE(dataset_connect_MyInt2)::EXOS_ERROR_CODE)::Ptr{Cchar})
