...
The Code generator only supports the first level of datasets for generating `PUB/SUB` datasets. 
...
The terms **Instatiatable** and **Singleton** refer to the possibility of instantiating the function blocks of the Library, i.e. have more than one copy of the data structure (data model) in the AR solution.

For both variants, the Datatype which is used to generate the template with is attached to the library as an external PV, which allows the user to use this structure as a local or a global PV in the AR solution.

## Instantiatable C Template

- Automation Runtime

    In Automation Runtime, the dynamic library (which has the name of the Datatype) has the possiblity to instantiate its function blocks, that local copies of the datatype can be used throughout the application. The exos-api functions are implemented directly in the Library, meaning it is rather simple for the user to add special features of the application using the C-API. Regenerating the template will however remove these added features.

- Linux

    In linux a C-executable is created together with a cmake instruction. Like the Automation Runtime library, the exos-api functions are used directly in the code, making it simple for the user to add special communication features. Regenerating the template will however reset these added features.

## Instantiatable N-API NodeJS Template

- Automation Runtime

    In Automation Runtime, the template is equivalent to the **Instantiatable C Template**

- Linux

    In Linux, this template uses the n-api (Node-API) to generate a complete nodejs object of the provided datamodel and `PUB/SUB` datasets. The Linux application consists of a `.deb` package of the compiled dataobject module, and a `.js` file with the user application. When exported to a binary format, the `.js` user application can be changed without having a build environment installed.
...
- Automation Runtime 
    ...
    Here a static library is created as a datamodel proxy, providing a singleton structure representing the datatype of the `.typ` file. The library includes all needed exos-api functions, and can be regenerated by right clicking on the `.exospkg` file.

    The dynamic library uses the datamodel proxy and only provides a single `Cyclic` function which can only be used on one location in the AR appliation.

    The benefit of this approach is that the program using the static library is decoupled from the exos-api (simplifying the "main" code), and only needs to access simplified datamodel functions to read and write values, and that the number of `PUB/SUB` members can be changed by regenerating the library. 

    The drawback of this template is that it only allows the usage of a single datamodel within one application, that is, the static library does not allow instantiation of the datamodel.

- Linux

    In Linux, the same static datamodel proxy library is created as in Automation Runtime. Here a C-executable is created that accesses the singleton structure, which is simpler than the **Instantiatable C Template**, but only allows accessing a single instance of the datamodel per application.

## Singleton SWIG Python Template
