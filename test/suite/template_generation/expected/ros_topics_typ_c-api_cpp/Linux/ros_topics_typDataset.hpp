...
#endif
#ifndef _ROS_TOPICS_TYPDATASET_H_
#define _ROS_TOPICS_TYPDATASET_H_
...
private:
    ...
    static void _datasetEvent(exos_dataset_handle_t *dataset, EXOS_DATASET_EVENT_TYPE event_type, void *info) {
        ros_topics_typDataset* inst = static_cast<ros_topics_typDataset*>(dataset->user_context);
        inst->datasetEvent(dataset, event_type, info);
    }

public:
    ros_topics_typDataset() {};
    
    T value;
    int nettime;
    void init(exos_datamodel_handle_t *datamodel, const char *browse_name, ros_topics_typLogger* _log) {
        log = _log;
    ...
    void connect(EXOS_DATASET_TYPE type) {
        exos_assert_ok(log, exos_dataset_connect(&dataset, type, &ros_topics_typDataset::_datasetEvent));
    };
    void publish() {
